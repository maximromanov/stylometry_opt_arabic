...
thisFileName <- "testing_for_optimal_parameters_loopSafe.R"

"
RUNNING MODEL: Looping through all parameters, but one-distance-one-linkage at a time
- the state of the script will be saved with the same filename as generated results
"
...
# FOLDERS
corpusFolder <- "/Users/romanovienna/_ADHFANIS_Data/chapter_3/Hindawi_Corpus_Wrapped/"
setwd("/Users/romanovienna/_ADHFANIS_Data/chapter_3/testing_halves/")
savePath = "/Users/romanovienna/_ADHFANIS_Data/chapter_3/testing_halves/slice_clustering_tests/"
...
# SMALL LIST
# listOfFiles <- c("1392TahaHusayn_JannatHayawan.txt",
...
# FINAL RESULTS FILES
fileNameBase = paste0("Hindawi_AllTexts_",format(Sys.time(), "D%Y%m%dT%H%M%S"))
fileNameTSV <- paste0(fileNameBase,".csv")

# SAVING THE STATE OF THE SCRIPT AS A EASY WAY TO PRESERVE PARAMETERS WITH WITH DAATA WAS PRODUCED
fileNameRscriptState <- paste0(fileNameBase,".R")
...
finalResults <- rbind(finalResults,
                      c("author_book", "clustering_method", "book_cluster_match",
...
write_tsv(finalResults, paste0(savePath,fileNameTSV), na = "NA", append = FALSE, col_names = FALSE)
finalResults = data.frame()
        ...
        for (sliceLength in sliceLengthV){
          for (mffLimit in mffLimitV){
            for (culling in cullingV){
              ...
              message("PROCESSING: ", featureType, nGramSize,
                      " SLICE:", sliceLength,
                      " MFF:", mffLimit,
                      " CULLING:", culling,
                      " DIST:", distanceMethod,
              ...
              if(length(clustered.data) > 0) {
                ...
                # cummulative fileNameTSV: must be defined in the variable section --- this one cannot be automatically generated
                write_tsv(finalResults, paste0(savePath,fileNameTSV), na = "NA", append = TRUE, col_names = FALSE,
                          quote_escape = "double")
